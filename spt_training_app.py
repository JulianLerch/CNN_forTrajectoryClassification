...
class TrainingApp:
    ...
    def start_training(self) -> None:
        ...
        def worker():
            try:
                ...
                stages = [
                    ("Generiere Trainingsdaten", self._run_data_generation, dict(samples=samples, mode=mode, ratio=ratio, polymer=polymer)),
                    ("Baue Modell", self._run_build_model, {}),
                    ("Trainiere Modell", self._run_training, dict(epochs=epochs, batch=batch)),
                ...
                self.queue.put(("done", True, str(output_dir)))
    ...
    def _run_data_generation(self, samples, mode, ratio, polymer) -> None:
        ...
        with redirect_stdout(stream), redirect_stderr(stream):
            self.trainer.generate_training_data(**kwargs)
    ...
    def _run_training(self, epochs, batch) -> None:
        ...
        stream = QueueStream(self._queue_log)
        with redirect_stdout(stream), redirect_stderr(stream):
            self.trainer.train(epochs=epochs, batch_size=batch, verbose=0, epoch_callback=epoch_update)
    ...
    def _run_save(self) -> None:
        ...
        stream = QueueStream(self._queue_log)
        with redirect_stdout(stream), redirect_stderr(stream):
            self.trainer.save_model(verbose=True)
